...
def cmd_fixture(args):
    ...
    def carray(name, raw):
        rows = "\n".join("  " + ", ".join(f"0x{x:02x}" for x in raw[i:i + 12]) + ","
                         for i in range(0, len(raw), 12))
        return f"static const uint8_t {name}[{len(raw)}] = {{\n{rows}\n}};\n"
    ...
    text = (
        ...
        "#pragma once\n\n#include <stdint.h>\n\n"
        + carray("MANIFEST_FIXTURE", blob) + "\n"
        + carray("FIXTURE_ROOT_KEY", public_of(root)) + "\n"
        + carray("FIXTURE_DELEGATE_KEY", public_of(delegate)))
    out = Path(args.out)
    if args.check:

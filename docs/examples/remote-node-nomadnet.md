...
4. On the phone (Sideband, connected to the node over TCP), the remote peer
   appears in the announce stream while the node's *Client interface mode* is
   `full`, which is the default; in `access_point` mode, add the address
   manually — Sideband will path-request it through the node.

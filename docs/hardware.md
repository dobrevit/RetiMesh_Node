...
to learn: it is an expansion module — the pin runs to a slot, and the bench unit's
slots are empty. A fitted module should sound without a firmware change. What
follows is what that first session had to know; a new unit re-checks the same list.

**The expansion modules, fitted 2026-09-10.** The slots are no longer empty, and what
went into them changes what this board reports. The sounder works as predicted, with no
firmware change. Two environmental parts came with it, and the bus scan is what named
them — `STATUS` prints `i2c main sda=4 scl=3 parts=3 acked=0x26 0x70 0x76`:

* **0x76 — a BME280**, temperature, humidity and pressure. The same part the T-Beam
  Supreme carries, and the same driver.
* **0x70 — a GXHT3V**, temperature and humidity. GXCAS's clone of Sensirion's SHTC3, and
  the address is what identifies the family: an SHT3x would be at 0x44 or 0x45 and speak
  a different command set.
* 0x26 is the DA217 accelerometer, which was already there. The touch controller is on
  the other host, at 0x2e.

**Both are read and both are reported** — on the console as `env` and `env2`, in
`/api/status` as two objects, on the portal as two cards, and as two rows on the home
screen. They are not merged into one temperature. They measure the same two quantities to
their own tolerances and will differ by a few tenths of a degree, and that difference is
the most useful thing this board can say about its own measurements: a firmware that
picked a winner would hide it and call the survivor "the temperature". Which part a
number came from is printed beside it.

Neither is accepted on the strength of an address. The BME280 has to answer chip id 0x60
*and* give up a calibration block with no zero coefficients; the humidity part has to
return a reply whose CRC-8 checks out, and an SHTC3 its product-code word as well. That is
what makes it safe to go looking: a module strapping the barometer to 0x77, or carrying an
SHT3x instead, is found without a build flag, and something else answering at one of those
addresses is refused rather than decoded into a plausible reading. A V4 with empty slots
costs nothing for any of this — the parts are looked for once at boot and reported absent.

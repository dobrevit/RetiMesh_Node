...
| USB networking (CDC-NCM) | on | offered only where the board has native USB **and** the build carries the driver — neither is true on the pinned core, so the switch is greyed out with the reason |
| PPP over the serial bridge | off | likewise, for the CP2102/CH9102 boards |
...
| `BOARD_USB_NATIVE`, `BOARD_USB_NCM`, `BOARD_USB_BRIDGE`, `BOARD_BRIDGE_AUTO_RESET`, `BOARD_UART_NETWORK`, `BOARD_UART_MAX_BAUD` | from `boards.json` | host connectivity of the PCB, generated by `tools/board_caps.py` — not set by hand |
| `LOCAL_LINK_USB_NCM`, `LOCAL_LINK_PPP` | 0 / 0 | whether the build carries the USB-NCM and PPP drivers (neither exists on the pinned core; see [local-link.md](local-link.md)) |

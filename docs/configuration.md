...
| `PPP_RX_RING_BYTES`, `PPP_TX_QUEUE_BYTES`, `PPP_BAUD_DEFAULT` | 4096 / 4096 / 115200 | PPP over the bridge UART: the UART driver's receive ring (drops on overflow), the transmit queue (a frame that does not fit is dropped whole), the speed a fresh node stores |

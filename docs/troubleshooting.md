...
| Opening the serial console resets the node | the terminal asserts DTR/RTS, which is the reset handshake on USB-Serial/JTAG and pulls EN on a bridge | open with DTR/RTS low (`retimesh-flash devices`, `pio device monitor --no-reconnect --rts 0 --dtr 0`) |
